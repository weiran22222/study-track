...
清理事实，以及 PR #28 已发生的远程负向失败与六字段诊断。

探针文件目前只在本地工作树删除；恢复后的远程成功、功能合并与最终 `main` 均尚未观察，
不能由本地状态推断。按照决策卡 011 的精简三级流程，本计划将在远程恢复实际完成后、
功能 PR 合并前归档；其余远程事实以实际 GitHub PR 与 Actions 记录为权威。
